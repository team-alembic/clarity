defmodule Atlas.PageLive do
    ...
    <nav class="tabs border-b border-base-light-300 dark:border-base-dark-700 bg-base-light-100 dark:bg-base-dark-900 px-4">
      <ul class="flex space-x-2">
        <li :for={content <- @contents}>
          <.link
            patch={Path.join([@prefix, Atlas.Vertex.unique_id(@current_vertex), content.id])}
            class={
  ...
  defp update_dynamics(socket, current_vertex, current_content) do
    %Atlas{graph: graph, root: root} = socket.assigns.atlas
      ...
      %Vertex.Content{
        ...
        content:
          {:viz,
           fn %{theme: theme} ->
             graph
             |> Atlas.GraphUtil.subgraph_within_steps(current_vertex, 2, 1)
             |> Atlas.Graph.to_dot(theme: theme, highlight: current_vertex)
           end}
      }
      | contents
    ]
