...
defmodule Atlas.GraphUtil do
  ...
  **`max_incoming_steps` backward hops**, together with every edge that connects
  ...
  ## Returns
    ...
    - A tree structure (`Atlas.tree()`) representing the hierarchy of the graph starting from the root vertex.
  ...
  @spec collect_tree_info(
          graph :: :digraph.graph(),
          queue :: :queue.queue(),
          visited_map :: %{required(:digraph.vertex()) => Atlas.tree()}
        ) :: %{required(:digraph.vertex()) => Atlas.tree()}
