defmodule Atlas.CoreComponents do
  @moduledoc false

  use Phoenix.Component

  import Atlas.Router, only: [__asset_path__: 2]
  import Phoenix.HTML
        ...
        <ol class="flex flex-wrap text-sm text-gray-400 space-x-2">
          <%= for {breadcrumb, idx} <- Enum.with_index(@breadcrumbs), idx > 0 do %>
            <li class="flex items-center">
              ...
              <.link
                patch={Path.join([@prefix, Atlas.Vertex.unique_id(breadcrumb), "graph"])}
  ...
  defp navigation_node(assigns) do
    ...
    <%= if Enum.any?(@tree.children, &(elem(&1, 0) != :content)) do %>
      <details open={Enum.any?(@breadcrumbs, &(&1 == @tree.node))}>
        <summary>
          <.link
            patch={Path.join([@prefix, Atlas.Vertex.unique_id(@tree.node), "graph"])}
            class={
