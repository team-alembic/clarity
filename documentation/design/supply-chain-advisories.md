...
2.5. **(done)** Dev-only **update action**, via a shared
   `Clarity.Content.DependencyUpdate` button embedded by both the Version Status
   tab (update an outdated dep to latest) and the Advisories tab (update to
